...
//Rare exceptions to indicate a bug in the _interface code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_interface = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_interface]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.interface.module"
